...
app.post('/', (req, res) => {
    ...
    async function functAsync(){
        ...
        calendar.events.list({
            ...
            if (events.length) {
            console.log('Upcoming 20 events:', events);
            events_arr=[];
            events.map((event, i) => {
    ...
    functAsync()
    fs.truncate('/token.json', 0, function(){console.log('done')})
    }
    res.send(events_arr)
})
...
app.post('/events', (req, res) => {
    ...
    // Create a new event start/end date instance
